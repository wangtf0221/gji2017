...
set(gca,'ylim',[1 1.32],'ytick',[1:0.1:1.4],'yticklabel','1.0|1.1|1.2|1.3');
...
%for i=2450:50:2550
	...
	B=pi/180*0;
	B1=asin(vs/vp*sin(B));
	eq1=h/cos(B)/vp+h/cos(B1)/vs;
	eq2=h*(tan(B)+tan(B1));
    r1=vs_dem/vp_dem;
	syms z1 A1 
%	[z1, A1]=solve()
	[A1,	z1]=solve(z1/cos(A1)/vp_dem+z1/cos(asin(r1*sin(A1)))/vs_dem==eq1,z1*(tan(A1)+tan(asin(r1*sin(A1))))==eq2)
%    Depth_dem=(vs_dem/vs*vp_dem/vp*(vp+vs)/(vp_dem+vs_dem))*h;
	Depth_dem=z1;
    sx=200;rx=1000;
...
%    Depth_dem=(vs_dem/vs*vp_dem/vp*(vp+vs)/(vp_dem+vs_dem))*h;
    ...
    hold on
	plot(X1,t1,'k--')
	annotation(fid,'rectangle',[0.3251 0.658 0.387 0.160],'LineWidth',2,'LineStyle','--','FaceColor','flat','Color',[1,0,0]);
%	h1=rectangle('Position',[0.3 0.5 0.37 0.16]);
%	set(h1,'edgecolor','r');
%	Box1=-0.5:0.01:0.5;
%	plot(Box1,1.05,'r--')
%end
